...
function SendButton() {
  ...
  return (
    <button type="submit" disabled={pending}
      className="w-full bg-forest text-paper text-[15px] font-semibold px-5 py-3 rounded-btn hover:bg-forest-hover transition-colors disabled:opacity-50">
...
export default function WholesaleInquirySheet({ farm, user, viewerBusinesses = [] }) {
  ...
  return (
    <section className="scroll-mt-20">
      <h2 className="font-serif text-[22px] sm:text-[26px] font-medium text-ink mb-4">Wholesale</h2>
      <div className="bg-card border border-hair rounded-panel p-5 sm:p-6 shadow-[0_1px_8px_rgba(30,21,9,0.04)]">
        <div className="flex items-center gap-2 text-[14px] text-soil mb-2">
          <span className="text-forest">✓</span> Available for wholesale
        </div>
        ...
        {state.success ? (
          <p className="text-[14px] font-medium text-forest">Inquiry sent. They’ll see it in their dashboard.</p>
        ) : !user ? (
          <p className="text-[13px] text-stone">
            <Link href={`/login?next=/producers/${farm.slug}`} className="font-semibold text-brick hover:underline">Sign in</Link>
            {' '}to request wholesale information.
          </p>
        ) : (
          <button type="button" onClick={() => setOpen(true)}
            className="bg-transparent text-forest text-[15px] font-semibold px-5 py-2.5 rounded-btn border border-forest hover:bg-sage-wash transition-colors">
      ...
      {open && user && !state.success && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
          ...
          <div role="dialog" aria-labelledby="inq-title" className="relative bg-card rounded-panel max-w-[440px] w-full p-7 shadow-xl">
            ...
            <form action={formAction} className="flex flex-col gap-3">
              ...
              <div>
                <h3 id="inq-title" className="font-serif text-[24px] font-medium text-ink">Request Wholesale Information</h3>
                <p className="text-[13px] text-stone mt-1">A note to {farm.name}. Not an order.</p>
              </div>
              {state.error && <p className="text-[13px] text-brick">{state.error}</p>}
              <label className="text-[11px] font-semibold tracking-wide uppercase text-stone">
                ...
                <input name="subject" placeholder="Sourdough, focaccia, granola…"
                  className="mt-1 w-full bg-card rounded-btn border border-hair px-3 py-2.5 text-[14px] font-normal normal-case text-soil outline-none focus:bg-white focus:border-gold" />
              </label>
              <label className="text-[11px] font-semibold tracking-wide uppercase text-stone">
                Quantity
              ...
              <label className="text-[11px] font-semibold tracking-wide uppercase text-stone">
                ...
                <textarea name="message" rows={4} placeholder="Who you are and what you need"
                  className="mt-1 w-full bg-card rounded-btn border border-hair px-3 py-2.5 text-[14px] font-normal normal-case text-soil outline-none focus:bg-white focus:border-gold resize-y" />
              </label>
              <SendButton />
            </form>
          </div>
