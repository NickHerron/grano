...
// components need the answer. Defaults to false if the settings row is ever missing,
// so a missing row can never silently take checkout live.
export const getLiveMarketplaceEnabled = cache(async () => {
  ...
  return data?.live_marketplace_enabled ?? false
})
